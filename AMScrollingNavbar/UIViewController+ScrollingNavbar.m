...
- (BOOL)isDraging{
    if(self.scrollableView){
        return self.panGesture.state == UIGestureRecognizerStateChanged;
    }
    return NO;
}
...
- (CGFloat)progressOnShowNavBar{
    if(self.isDraging){
        return (self.navigationController.navigationBar.frame.origin.y+self.deltaLimit)/(self.statusBar+self.deltaLimit+0.0);
    }else if(self.isShowingNavBar){
        return 1;
    }else {
        return 0;
    }
